...
package com.kss.social.processors;
...
import com.kss.social.adapters.FoursquareConnectorLifecycleAdapter;
...
/**
 * SearchVenuesMessageProcessor invokes the {@link com.kss.social.FoursquareConnector#searchVenues(java.lang.String, java.lang.String, java.lang.String)} method in {@link FoursquareConnector }. For each argument there is a field in this processor to match it.  Before invoking the actual method the processor will evaluate and transform where possible to the expected argument type.
 * 
 */
public class SearchVenuesMessageProcessor
    implements FlowConstructAware, MuleContextAware, Disposable, Initialisable, Startable, Stoppable, MessageProcessor
{

    private Object lat;
    private String _latType;
    private Object lng;
    private String _lngType;
    private Object query;
    private String _queryType;
    private static Logger logger = LoggerFactory.getLogger(SearchVenuesMessageProcessor.class);
    /**
     * Module object
     * 
     */
        ...
        throws InitialisationException
    {
        ...
        if (moduleObject == null) {
            try {
                moduleObject = muleContext.getRegistry().lookupObject(FoursquareConnectorConnectionManager.class);
                if (moduleObject == null) {
                    throw new InitialisationException(MessageFactory.createStaticMessage("Cannot find object"), this);
                }
            } catch (RegistrationException e) {
                throw new InitialisationException(CoreMessages.initialisationFailure("com.kss.social.adapters.FoursquareConnectorConnectionManager"), e, this);
            }
        }
        ...
        throws MuleException
    {
        ...
        FoursquareConnectorConnectionManager _castedModuleObject = null;
        if (moduleObject instanceof String) {
            _castedModuleObject = ((FoursquareConnectorConnectionManager) muleContext.getRegistry().lookupObject(((String) moduleObject)));
            if (_castedModuleObject == null) {
                throw new MessagingException(CoreMessages.failedToCreate("searchVenues"), event, new RuntimeException("Cannot find the configuration specified by the config-ref attribute."));
            }
        } else {
            _castedModuleObject = ((FoursquareConnectorConnectionManager) moduleObject);
        }
        FoursquareConnectorLifecycleAdapter connection = null;
        try {
            String _transformedLat = ((String) evaluateAndTransform(_muleMessage, SearchVenuesMessageProcessor.class.getDeclaredField("_latType").getGenericType(), null, lat));
            String _transformedLng = ((String) evaluateAndTransform(_muleMessage, SearchVenuesMessageProcessor.class.getDeclaredField("_lngType").getGenericType(), null, lng));
            String _transformedQuery = ((String) evaluateAndTransform(_muleMessage, SearchVenuesMessageProcessor.class.getDeclaredField("_queryType").getGenericType(), null, query));
            ...
            connection = _castedModuleObject.acquireConnection(new FoursquareConnectorConnectionManager.ConnectionKey());
            if (connection == null) {
                throw new MessagingException(CoreMessages.failedToCreate("searchVenues"), event, new RuntimeException("Cannot create connection"));
            } else {
                if (logger.isDebugEnabled()) {
                    StringBuilder _messageStringBuilder = new StringBuilder();
            ...
            resultPayload = connection.searchVenues(_transformedLat, _transformedLng, _transformedQuery);
            ...
            throw new MessagingException(CoreMessages.failedToInvoke("searchVenues"), event, e);
        } finally {
            try {
                if (connection!= null) {
                    if (logger.isDebugEnabled()) {
                    ...
                    _castedModuleObject.releaseConnection(new FoursquareConnectorConnectionManager.ConnectionKey(), connection);
                }
            } catch (Exception e) {
                throw new MessagingException(CoreMessages.failedToInvoke("searchVenues"), event, e);
            }
